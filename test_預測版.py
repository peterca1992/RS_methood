...
#%%
#加上預測用的價錢
df_delta = pd.concat([df_delta, pd.DataFrame(df_delta.iloc[-1]).T], axis = 0).reset_index(drop = True)
df_price = pd.concat([df_price, pd.DataFrame(df_price.iloc[-1]).T], axis = 0).reset_index(drop = True)
df_model_price = pd.concat([df_model_price, pd.DataFrame(df_model_price.iloc[-1]).T], axis = 0).reset_index(drop = True)

df_delta["日期"].iloc[-1] = df_delta["日期"].iloc[-2] + pd.DateOffset(1)
df_price["日期"].iloc[-1] = df_price["日期"].iloc[-2] + pd.DateOffset(1)
df_model_price["日期"].iloc[-1] = df_model_price["日期"].iloc[-2] + pd.DateOffset(1)

#更新價格
for i in df_price.columns[1:]:
    
    df_price[i].iloc[-1] = df_price[i].iloc[-2]*1.05

for i in df_model_price.columns[1:]:
    
    df_model_price[i].iloc[-1] = df_model_price[i].iloc[-2]*1.05
...
for t in range(t_start, len(df_price)):
    ...
    #先找出當日前三百大市值
    stock_300 = df_delta[df_delta["日期"] == df_price["日期"].iloc[t]].T.iloc[1:].reset_index(drop = False).sort_values(by = t, ascending = False).iloc[:400].reset_index(drop = True)
